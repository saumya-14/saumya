...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  return (
    <div
      ...
      ref={containerRef}
    >
      <div className="max-w-7xl mx-auto py-5 px-4   flex justify-center items-center">
        ...
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col z-40 items-center top-40 self-start max-w-xs">
              ...
              <h3 className="hidden  text-xl  font-bold text-blue-500 ">
                {item.title}
              </h3>
            </div>

            <div className="relative pl-20 pr-4 w-full">
              <h3 className=" block text-2xl mb-4 text-left font-bold text-purple">
                {item.title}
              </h3>
              {item.content}{" "}
            </div>
